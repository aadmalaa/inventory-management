...
.items-summary::before {
  content: '▶';
  display: inline-block;
  margin-right: var(--space-1); /* was 0.375rem (6px): snapped down to the 4px scale */
  font-size: 0.75rem;
...
.items-dropdown {
  ...
  box-shadow: var(--shadow-md); /* was a two-layer ad-hoc shadow: snapped to the dropdown token */
  padding: var(--space-3);
  z-index: 10;
  min-width: 300px;
  max-width: 400px;
...
.item-entry {
  ...
  gap: var(--space-1);
  padding: var(--space-2);
  border-bottom: 1px solid var(--color-hover); /* #f1f5f9 doubles as the hairline row divider */
}
